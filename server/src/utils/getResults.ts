import { NominationID, Nominations, Rankings, Results } from 'shared';
...
export default (
  ...
  // 1. Each value of rankings key values is an array of participants' vote.
  // Points for each array element corresponds to following formula:
  // r_n = ((votesPerVoter - 0.5*n) / votesPerVoter)^(n+1), where n corresponds
  // to the index of rankings.
  // Accumulate scores per nominationID.
  const scores: { [nominationID: NominationID]: number } = {};
