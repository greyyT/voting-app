...
const Results: React.FC = () => {
  ...
  return (
    <>
      <div className="mx-auto flex flex-col w-full justify-between items-center h-full max-w-sm">
        ...
        <div className="flex flex-col justify-center">
          {isAdmin && !poll?.results.length && (
            <>
              <button className="box btn-orange my-2" onClick={() => setIsConfirmationOpen(true)}>
                End Poll
              </button>
            </>
          )}
          {!isAdmin && !poll?.results.length && (
            <div className="my-2 italic">
              Waiting for Admin <span className="font-semibold">{poll?.participants[poll?.adminID]}</span> to finalize
              the poll.
            </div>
          )}
          {!!poll?.results.length && (
      ...
      {isLeavePollOpen && (
        <ConfirmationDialog
          message="You'll lose your results. Is that alright?"
