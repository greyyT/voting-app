...
const makeRequest = async <T>(endpoint: string, reqInit?: RequestInit): Promise<MakeRequestResponse<T>> => {
  try {
    ...
    const error =
      e instanceof Error
        ? {
            messages: [import.meta.env.MODE === 'development' ? e.message : 'Unknown error'],
          }
        : {
