...
const BottomSheet: React.FC<BottemSheetProps> = ({ isOpen = false, onClose, children }) => {
  return (
    <CSSTransition
      in={isOpen}
      timeout={300}
    ...
    >
      <div className="absolute left-0 right-0 max-w-screen-sm bg-gray-50 bottom-0 z-10 overflow-y-hidden top-16 flex flex-col">
        ...
        <div className="relative overflow-y-hidden bg-gray-50 flex-grow">
          <div className="absolute top-0 bottom-0 left-0 right-0 overflow-y-auto">{children}</div>
        </div>
      </div>
    </CSSTransition>
  );
